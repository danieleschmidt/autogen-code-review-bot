...
def create_task_interactive(planner: QuantumTaskPlanner) -> None:
    ...
    priority_input = (
        input("Priority bias (optional, press Enter to skip): ").strip().upper()
    )
    ...
    if priority_input:
        try:
            ...
            bias_strength = float(
                input("Bias strength (0.1-1.0, default 0.3): ") or "0.3"
            )
            ...
            print(
                "⚠️  Invalid priority or bias strength, using default quantum superposition"
            )
...
def load_tasks_from_json(planner: QuantumTaskPlanner, filename: str) -> None:
    ...
    try:
        ...
        for task_data in tasks_data:
            ...
            if "priority_bias" in task_data:
                ...
                try:
                    ...
                    print(
                        f"⚠️  Unknown priority '{priority_name}' for task {task_data['id']}"
                    )
        ...
        for entanglement in entanglements:
            try:
                planner.create_task_entanglement(
                    entanglement["task1"], entanglement["task2"]
                )
        ...
        print(
            f"✅ Loaded {created_tasks} tasks and {created_entanglements} entanglements from {filename}"
        )
...
def display_execution_plan(plan: Dict) -> None:
    ...
    for i, task in enumerate(plan["scheduled_tasks"], 1):
        ...
        entangled_info = (
            f" [⚛️  {len(task['entangled_with'])} entangled]"
            if task["entangled_with"]
            else ""
        )
        deps_info = (
            f" [📎 {len(task['dependencies'])} deps]" if task["dependencies"] else ""
        )

        print(f"{i:2d}. {priority_icon} {task['title']}")
        print(
            f"     ID: {task['id']} | Effort: {task['estimated_effort']:.1f}h{deps_info}{entangled_info}"
        )
        if task["description"]:
...
""",
    ...
    parser.add_argument(
        "--load", "-l", metavar="JSON_FILE", help="Load tasks from JSON file"
    )

    # Action options
    parser.add_argument(
        "--plan", "-p", action="store_true", help="Generate quantum execution plan"
    )
    ...
    parser.add_argument(
        "--analytics", "-a", action="store_true", help="Show planning analytics"
    )
