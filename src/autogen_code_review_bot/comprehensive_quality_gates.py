...
from .advanced_validation import get_advanced_validator, ValidationSeverity
...
class QualityGateResult(BaseModel):
    ...
    def is_critical_failure(self) -> bool:
        ...
        return (
            self.status == QualityGateStatus.FAILED and
            self.gate_type in [
...
class ComprehensiveQualityGates:
    ...
    def __init__(self, repo_path: str = "."):
        ...
        logger.info("Comprehensive quality gates system initialized", repo_path=str(self.repo_path))
    
    def _setup_quality_gates(self):
        """Setup all quality gate implementations"""
        
        # Code quality gates
        self.quality_gates[QualityGateType.CODE_QUALITY] = self._execute_code_quality_gate
        self.quality_gates[QualityGateType.TEST_COVERAGE] = self._execute_test_coverage_gate
        
        # Security gates
        self.quality_gates[QualityGateType.SECURITY_SCAN] = self._execute_security_scan_gate
        self.quality_gates[QualityGateType.DEPENDENCY_CHECK] = self._execute_dependency_check_gate
        
        # Performance gates
        self.quality_gates[QualityGateType.PERFORMANCE_BENCHMARK] = self._execute_performance_benchmark_gate
        self.quality_gates[QualityGateType.LOAD_TEST] = self._execute_load_test_gate
        
        # Compliance and documentation gates
        self.quality_gates[QualityGateType.COMPLIANCE_CHECK] = self._execute_compliance_check_gate
        self.quality_gates[QualityGateType.DOCUMENTATION_CHECK] = self._execute_documentation_check_gate
        
        # Testing gates
        self.quality_gates[QualityGateType.INTEGRATION_TEST] = self._execute_integration_test_gate
    ...
    async def execute_quality_gates(
        ...
        logger.info("Executing quality gates suite",
            ...
            for gate_type, task in tasks:
                try:
                    ...
                    if result.is_critical_failure():
                        critical_failures += 1
                        if fail_fast:
                            logger.error("Critical quality gate failure, stopping execution",
                    ...
                    logger.error("Quality gate execution error",
                               gate_type=gate_type.value, error=str(e))
        ...
        logger.info("Quality gates suite completed",
                   overall_score=overall_score,
                   compliance_level=compliance_level,
                   critical_failures=critical_failures,
                   execution_time=execution_time)
    ...
    async def _execute_single_gate(self, gate_type: QualityGateType) -> QualityGateResult:
    ...
    async def _execute_code_quality_gate(self) -> QualityGateResult:
        ...
        if mypy_result["success"] or mypy_result["returncode"] == 1:  # MyPy returns 1 for type errors
            mypy_errors = ruff_result["output"].count("error:") if ruff_result["output"] else 0
        ...
        total_issues = quality_checks.get("ruff_issues", 0) + quality_checks.get("mypy_errors", 0)
    ...
    async def _execute_security_scan_gate(self) -> QualityGateResult:
        ...
        status = QualityGateStatus.PASSED if critical_vulnerabilities <= threshold else QualityGateStatus.FAILED
    ...
    async def _execute_dependency_check_gate(self) -> QualityGateResult:
        ...
        status = QualityGateStatus.PASSED if critical_vulns <= threshold else QualityGateStatus.WARNING
    ...
    async def _execute_load_test_gate(self) -> QualityGateResult:
        ...
        status = QualityGateStatus.PASSED if error_rate <= 5.0 else QualityGateStatus.WARNING
    ...
    async def _execute_documentation_check_gate(self) -> QualityGateResult:
        ...
        docs_score = sum(1 for check in docs_checks.values() if check) / len(docs_checks) * 100
        status = QualityGateStatus.PASSED if docs_score >= 70.0 else QualityGateStatus.WARNING
    ...
    async def _execute_integration_test_gate(self) -> QualityGateResult:
        ...
        status = QualityGateStatus.PASSED if integration_result["success"] else QualityGateStatus.FAILED
    ...
    async def _execute_smoke_test_gate(self) -> QualityGateResult:
        ...
        status = QualityGateStatus.PASSED if smoke_result["success"] else QualityGateStatus.FAILED
