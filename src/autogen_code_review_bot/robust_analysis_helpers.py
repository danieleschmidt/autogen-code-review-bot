...
from .robust_error_handling import robust_operation, ErrorSeverity, validate_file_path, safe_execute
...
def run_security_analysis(repo_path: str, languages: List[str]) -> AnalysisSection:
    ...
    return AnalysisSection(
        ...
        output=output,
        return_code=0,
        metadata={"severity": severity, "tools_run": tools_attempted, "tools_succeeded": len(results)}
    )
