        ...
        try:
            ...
            if output_format == "json":
                output_data = {
                    ...
                    "performance": (
                        result.performance.output if result.performance else None
                    ),
...
@click.option(
    "--post-comment/--no-comment", default=False, help="Post results as PR comment"
)
@click.pass_context
def review_pr(ctx, repo, pr_number, token, post_comment):
    ...
    if not token:
        ...
        if not token:
            console.print(
                "❌ GitHub token required (use --token or GITHUB_TOKEN env var)",
                style="red",
            )
            sys.exit(1)
...
@click.option(
    "--conversation-rounds", type=int, default=3, help="Number of conversation rounds"
)
@click.option("--save-conversation", help="Save conversation log to file")
@click.pass_context
def agent_conversation(
    ctx, repo_path, agents_config, conversation_rounds, save_conversation
):
...
@click.option(
    "--interactive", "-i", is_flag=True, help="Interactive task planning mode"
)
...
def quantum_plan(ctx, task_file, interactive, export):
    ...
    if interactive:
        ...
        console.print(
            "Interactive mode not yet implemented. Use quantum-planner command for full CLI."
        )
        return
...
def _format_markdown_output(result, repo_path) -> str:
    ...
    md = "# AutoGen Code Review Results\n\n"
