...
class AuthenticationManager:
    ...
    def authenticate_request(self, token: str) -> Optional[APIUser]:
        ...
        try:
            ...
            self.logger.info(
                ...
                extra={"user_id": user_id, "organization": user.organization},
            )
    ...
    def generate_token(self, user_id: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.info(
                "JWT token generated",
                extra={"user_id": user_id, "expires_at": payload["exp"].isoformat()},
            )
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not rate_manager.check_rate_limit(user):
                req_logger.warning(
                    "Rate limit exceeded", extra={"user_id": user.user_id}
                )
                return (
                    jsonify(
                        {
...
def generate_auth_token():
    ...
    try:
        ...
        if not data or "user_id" not in data:
            return (
                jsonify({"error": "Invalid request", "message": "user_id required"}),
                400,
            )
        ...
        return jsonify(
            {
                "token": token,
                "expires_at": (
                    datetime.now(timezone.utc) + auth_manager.token_expiry
                ).isoformat(),
...
def analyze_repository():
    ...
    try:
        ...
        if not data or "repository_path" not in data:
            return (
                jsonify(
                    {"error": "Invalid request", "message": "repository_path required"}
                ),
                400,
            )
        ...
        if agent_config and Path(agent_config).exists():
            try:
                formatted_output = format_analysis_with_agents(
                    analysis_result, agent_config
                )
            except Exception as e:
...
def get_metrics():
    ...
    try:
        ...
        system_metrics = {
            ...
            "active_users": len(
                [u for u in auth_manager.users.values() if u.is_active]
            ),
            "total_requests_today": sum(
                v
                for k, v in rate_manager.usage_tracking.items()
                if k.endswith(str(datetime.now(timezone.utc).date()))
            ),
            "system_status": "operational",
            "uptime": (
                time.time() - app._start_time if hasattr(app, "_start_time") else 0
            ),
        }
        ...
        return jsonify({"error": "Metrics unavailable", "message": str(e)}), 500
...
def handle_file_too_large(e):
    ...
    return (
        jsonify(
            {"error": "File too large", "message": "Request size exceeds 50MB limit"}
        ),
        413,
    )
...
def create_api_app(config: Optional[Dict[str, Any]] = None) -> Flask:
    ...
    logger.info(
        "AutoGen API Gateway initialized",
        extra={
            "version": "1.0.0",
