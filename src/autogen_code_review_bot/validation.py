...
class InputValidator:
    ...
    def _register_default_rules(self):
        ...
        self.add_validation_rule(
            ...
            ValidationRule(
                name="scheme",
                validator=lambda x: urllib.parse.urlparse(str(x)).scheme
                in ["http", "https"],
    ...
    def sanitize(
        self, value: Any, data_type: str, custom_rules: List[SanitizationRule] = None
    ) -> Any:
    ...
    def validate_and_sanitize(
        ...
        custom_sanitization_rules: List[SanitizationRule] = None,
    ) -> Dict[str, Any]:
    ...
    def _get_json_depth(self, obj: Any, depth: int = 0) -> int:
        ...
        if isinstance(obj, dict):
            return (
                max(self._get_json_depth(value, depth + 1) for value in obj.values())
                if obj
                else depth
            )
        else:  # list
            return (
                max(self._get_json_depth(item, depth + 1) for item in obj)
                if obj
                else depth
            )
...
class SchemaValidator:
    ...
    def validate_schema(
        ...
        result = {"valid": True, "errors": [], "warnings": [], "field_results": {}}
    ...
    def _validate_field(
        ...
        if field_schema.get("trim", False):
            custom_sanitization_rules.append(
                SanitizationRule(
                    name="trim",
                    sanitizer=lambda x: str(x).strip(),
...
class SecurityValidator:
    ...
    def validate_ip_address(
        self, ip_str: str, allow_private: bool = False
    ) -> Dict[str, Any]:
...
def validate_and_sanitize_input(
    value: Any, data_type: str, required: bool = True
) -> Dict[str, Any]:
