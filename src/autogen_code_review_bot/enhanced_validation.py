...
class SecurityValidator:
    ...
    def validate_file_path(
            ...
            if base_path:
                try:
                    ...
                    raise ValidationError(
                        f"Path {file_path} is outside allowed base path {base_path}"
                    )
            else:
        ...
        if extension and extension not in cls.ALLOWED_CODE_EXTENSIONS:
            logger.warning(
                "Unusual file extension detected",
                file_path=file_path,
                extension=extension,
            )
            metrics.record_counter(
                "validation_unusual_extension", 1, tags={"extension": extension}
            )
    ...
    def validate_code_content(
        cls, content: str, filename: Optional[str] = None
    ) -> bool:
        ...
        if suspicious_found:
            logger.warning(
                "Suspicious patterns detected in code",
                filename=filename or "unknown",
                patterns=suspicious_found[:5],
            )  # Limit logged patterns
            metrics.record_counter(
                "validation_suspicious_patterns",
                1,
    ...
    @classmethod
    def sanitize_input(
        cls, value: str, max_length: int = 1000, allow_html: bool = False
    ) -> str:
...
class DataValidator:
    ...
    def validate_json(data: str, max_size: int = 1024 * 1024) -> Dict[str, Any]:
        ...
        if len(data.encode("utf-8")) > max_size:
            raise ValidationError(
                f"JSON data too large: {len(data)} bytes > {max_size}"
            )
    ...
    def validate_yaml(data: str, max_size: int = 1024 * 1024) -> Dict[str, Any]:
        ...
        if len(data.encode("utf-8")) > max_size:
            raise ValidationError(
                f"YAML data too large: {len(data)} bytes > {max_size}"
            )
...
def create_validation_schema() -> Dict[str, Any]:
    ...
    """
    return {
        "server": {"type": dict, "required": False},
        "github": {"type": dict, "required": True},
