...
from .quantum_planner import QuantumTask, TaskPriority, TaskState, QuantumScheduler, QuantumTaskPlanner
...
class TaskValidator:
    ...
    def validate_task_id(task_id: str) -> ValidationResult:
        ...
        if not task_id.replace('_', '').replace('-', '').replace('.', '').isalnum():
            result.add_warning("Task ID contains special characters (recommended: alphanumeric, _, -, .)")
    ...
    def validate_quantum_properties(task: QuantumTask) -> ValidationResult:
        ...
        if abs(total_amplitude_squared - 1.0) > 0.001:
            result.add_error(f"Priority amplitudes not normalized: {total_amplitude_squared:.6f}")
        ...
        for priority, amplitude in task.priority_amplitudes.items():
            if not isinstance(amplitude, (int, float)):
                ...
                result.add_warning(f"Amplitude for {priority.label} is greater than 1.0")
        ...
        for task_id, task in tasks.items():
            for dep_id in task.dependencies:
                if dep_id not in tasks:
                    result.add_error(f"Task '{task_id}' depends on non-existent task '{dep_id}'")
        ...
        for task_id, task in tasks.items():
            for entangled_id in task.entangled_tasks:
                if entangled_id not in tasks:
                    result.add_error(f"Task '{task_id}' entangled with non-existent task '{entangled_id}'")
                elif task_id not in tasks[entangled_id].entangled_tasks:
                    result.add_error(f"Asymmetric entanglement: '{task_id}' -> '{entangled_id}' but not vice versa")
...
class QuantumSystemValidator:
    ...
    def validate_system(self, scheduler: QuantumScheduler) -> ValidationResult:
        ...
        dep_result = self.dependency_validator.validate_dependencies_exist(scheduler.tasks)
        ...
        circular_result = self.dependency_validator.detect_circular_dependencies(scheduler.tasks)
        ...
        entangle_result = self.dependency_validator.validate_entanglements(scheduler.tasks)
        ...
        if len(scheduler.tasks) == 0:
            ...
            result.add_warning("Very large number of tasks (>10000) - performance may be impacted")
...
class QuantumDataIntegrity:
    ...
    def calculate_system_hash(scheduler: QuantumScheduler) -> str:
        ...
        system_data = {
            ...
            'quantum_circuits': sorted([sorted(circuit) for circuit in scheduler.quantum_circuits])
        }
    ...
    def verify_quantum_conservation(scheduler: QuantumScheduler) -> ValidationResult:
        ...
        for task_id, task in scheduler.tasks.items():
            ...
            if abs(total_prob - 1.0) > 0.001:
                result.add_error(f"Task '{task_id}' violates probability conservation: {total_prob:.6f}")
...
class RobustQuantumPlanner(QuantumTaskPlanner):
    ...
    def create_task(self, task_id: str, title: str, description: str, 
        ...
        if not task_result:
            ...
            raise ValidationError(f"Task creation failed validation: {', '.join(task_result.errors)}")
    ...
    def generate_execution_plan(self) -> Dict:
        ...
        if not validation_result:
            error_msg = f"System validation failed: {', '.join(validation_result.errors)}"
        ...
        integrity_result = self.integrity_checker.verify_quantum_conservation(self.scheduler)
        if not integrity_result:
            error_msg = f"Quantum conservation violated: {', '.join(integrity_result.errors)}"
            logger.error(error_msg)
            raise QuantumError(error_msg)
    ...
    def get_system_health(self) -> Dict:
        ...
        return {
            ...
            'entangled_tasks': sum(1 for task in self.scheduler.tasks.values() 
                                 if task.state == TaskState.ENTANGLED),
            'collapsed_tasks': sum(1 for task in self.scheduler.tasks.values() 
            ...
            for entangled_id in list(task.entangled_tasks):
                if entangled_id in self.scheduler.tasks:
                    ...
                    if task_id not in other_task.entangled_tasks:
                        ...
                        repair_log.append(f"Fixed asymmetric entanglement: {entangled_id} -> {task_id}")
                else:
                    # Remove entanglement to non-existent task
                    task.entangled_tasks.remove(entangled_id)
                    repair_log.append(f"Removed entanglement to non-existent task '{entangled_id}' from '{task_id}'")
