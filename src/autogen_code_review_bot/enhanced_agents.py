    ...
    def generate_review(self, analysis_data: str, context: Optional[str] = None) -> str:
        ...
        try:
            ...
            style_issues = [
                line
                for line in lines
                if "style" in line.lower() or "lint" in line.lower()
            ]
            performance_issues = [
                line
                for line in lines
                if "performance" in line.lower() or "complex" in line.lower()
            ]
...
"""
            ...
            if "architecture" in self.config.focus_areas:
                general_recommendations.append(
                    "- Review overall system design and modularity"
                )
                general_recommendations.append(
        ...
        try:
            ...
            for i, agent in enumerate(self.agents):
                ...
                self.conversation_log.append(
                    {"agent": agent.config.name, "round": 0, "content": review}
                )
...
def load_enhanced_agents_from_yaml(
    config_path: str,
) -> tuple[List[EnhancedAgent], ConversationConfig]:
    """Load enhanced agent configuration from YAML file."""
    try:
        if not Path(config_path).exists():
            logger.warning(
                f"Agent config file not found: {config_path}, using defaults"
            )
...
def _create_default_agents() -> tuple[List[EnhancedAgent], ConversationConfig]:
    ...
    coder_config = EnhancedAgentConfig(
        name="Senior Developer", focus_areas=["functionality", "bugs", "architecture"]
    )
...
def run_enhanced_dual_review(
    analysis_data: str, config_path: str = "config/agents.yaml"
) -> str:
