...
from .robust_error_handling import robust_operation, ErrorSeverity, validate_file_path
...
@robust_operation(component="security_analysis", operation="run_analysis", severity=ErrorSeverity.HIGH, retry_count=1)
...
@robust_operation(component="style_analysis", operation="run_analysis", severity=ErrorSeverity.MEDIUM, retry_count=2)
def run_style_analysis(repo_path: str, languages: List[str], linter_config: LinterConfig) -> AnalysisSection:
    ...
    try:
        ...
        if 'python' in languages:
            ...
            try:
                ...
                results.append(f"Python linter ({linter}): Tool not available or timed out")
        ...
        issues_count = len([r for r in results if "error" in r.lower() or "warning" in r.lower()])
...
@robust_operation(component="performance_analysis", operation="run_analysis", severity=ErrorSeverity.LOW, retry_count=1)
def run_performance_analysis(repo_path: str, languages: List[str]) -> AnalysisSection:
    ...
    try:
        ...
        try:
            ...
            if large_files:
                results.append(f"Large files detected:\n" + "\n".join(large_files[:10]))
        except Exception:
            pass
        ...
        try:
            ...
            for file_path in Path(repo_path).rglob('*'):
                if (file_path.is_file() and 
                    ...
                    file_path.suffix in {'.py', '.js', '.ts', '.go', '.java', '.cpp', '.c'}):
            ...
            if total_files > 0:
                ...
                results.append(f"Code metrics: {total_files} files, {total_lines} lines, avg {avg_lines:.1f} lines/file")
        except Exception:
            pass
        
        output = "\n\n".join(results) if results else "Performance analysis completed - no major concerns"
        hotspots = len([r for r in results if "complex" in r.lower() or "large" in r.lower()])
