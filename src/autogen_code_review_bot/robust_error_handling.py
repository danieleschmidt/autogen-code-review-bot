...
class RobustErrorHandler:
    ...
    def handle_error(self, error: Exception, context: ErrorContext) -> None:
        ...
        if context.severity == ErrorSeverity.CRITICAL:
            logger.critical(error_message, extra={
            ...
            logger.info(error_message, extra={
                'component': context.component,
                'operation': context.operation
            })
        ...
        def wrapper(*args, **kwargs):
            ...
            if error_handler.is_circuit_open(component, operation):
                if fallback_value is not None:
                    logger.warning(f"Circuit breaker open for {component}.{operation}, returning fallback")
                    return fallback_value
                elif raise_on_failure:
                    raise AnalysisError(f"Circuit breaker open for {component}.{operation}")
                else:
                    return None
            ...
            for attempt in range(retry_count + 1):
                try:
                    ...
                    if attempt < retry_count:
                        logger.info(f"Retrying {component}.{operation} (attempt {attempt + 2})")
                        continue
                    
                    # Last attempt failed
                    if fallback_value is not None:
                        logger.warning(f"All attempts failed for {component}.{operation}, returning fallback")
                        return fallback_value
                    elif raise_on_failure:
                        raise AnalysisError(f"Operation {component}.{operation} failed after {retry_count + 1} attempts: {e}")
                    else:
                        return None
    ...
    **kwargs
    ...
    try:
        ...
        if fallback_value is not None:
            ...
            raise AnalysisError(f"Safe execution failed for {component}.{operation}: {e}")
...
def validate_input(value: Any, validator: Callable[[Any], bool], error_message: str) -> None:
...
class HealthChecker:
    ...
    def check_component_health(self, component: str) -> dict:
        ...
        try:
            ...
            failure_count = sum(
                count for key, count in error_handler.failure_counts.items() 
            ...
            open_circuits = [
                key for key in error_handler.circuit_breakers 
    ...
    def get_overall_health(self) -> dict:
        ...
        try:
            components = ['pr_analysis', 'security_analysis', 'style_analysis', 'performance_analysis', 'agents']
            health_checks = {}
            ...
            return {
                ...
                'error_summary': {
                    ...
                    'circuit_breakers_open': len([cb for cb in error_handler.circuit_breakers.values() if cb]),
