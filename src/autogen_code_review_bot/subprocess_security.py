...
class SubprocessValidator:
    ...
    def validate_command(
        ...
        try:
            ...
            logger.error(
                "Unexpected error during command validation",
    ...
    def _validate_command_structure(cls, cmd: List[str]) -> None:
        ...
        if len(cmd) > cls.MAX_ARGUMENTS:
            raise ValidationError(
                f"Too many arguments: {len(cmd)} > {cls.MAX_ARGUMENTS}"
            )
        ...
        for i, arg in enumerate(cmd):
            ...
            if len(arg) > cls.MAX_ARGUMENT_LENGTH:
                raise ValidationError(
                    f"Argument {i} too long: {len(arg)} > {cls.MAX_ARGUMENT_LENGTH}"
                )
    ...
    def _validate_executable(cls, executable: str) -> None:
        ...
        try:
            ...
            if "/" in executable or "\\" in executable:
                ...
                if not any(
                    str(resolved_path).startswith(prefix) for prefix in allowed_prefixes
                ):
                    raise ValidationError(
                        f"Executable path not in allowed locations: {resolved_path}"
                    )
    ...
    def _validate_arguments(cls, cmd: List[str]) -> None:
        ...
        for i, arg in enumerate(cmd):
            ...
            for char in cls.DANGEROUS_CHARS:
                if char in arg:
                    raise ValidationError(
                        f"Dangerous character in argument {i}: {repr(char)}"
                    )
            ...
            for char in cls.SHELL_METACHARACTERS:
                if char in arg:
                    ...
                    if not cls._is_safely_quoted(arg):
                        raise ValidationError(
                            f"Shell metacharacter in argument {i}: {char}"
                        )
        ...
        if executable in cls.RESTRICTED_ARGUMENTS:
            ...
            for arg in cmd[1:]:
                if arg in restricted_args:
                    raise ValidationError(
                        f"Restricted argument for {executable}: {arg}"
                    )
    ...
    def _validate_working_directory(
        cls, cwd: str, project_root: Optional[str] = None
    ) -> None:
        ...
        for char in cls.DANGEROUS_CHARS:
            if char in cwd:
                raise ValidationError(
                    f"Dangerous character in working directory: {repr(char)}"
                )
        ...
        try:
            ...
            if project_root:
                ...
                try:
                    ...
                    raise ValidationError(
                        f"Working directory outside project root: {cwd}"
                    )
    ...
    def _validate_resource_limits(cls, cmd: List[str]) -> None:
        ...
        if cmd[0] in resource_intensive_commands:
            ...
            if "-timeout" not in cmd and "--timeout" not in cmd:
                logger.warning(
                    "Resource-intensive command without timeout",
                    extra={"command": cmd[0]},
                )
        ...
        try:
            ...
            if normalized != arg:
                ...
                if abs(len(normalized) - len(arg)) > 5:
                    raise ValidationError(
                        f"Unicode normalization attack in argument {arg_index}"
                    )
    ...
    def _validate_permission_command(cls, cmd: List[str]) -> None:
        ...
        for target in cmd[2:]:
            if (
                ...
                or target.startswith("/bin")
            ):
                raise ValidationError(
                    f"System file permission change attempted: {target}"
                )
...
def safe_subprocess_run(
    ...
    **kwargs,
    ...
    try:
        logger.debug(
            "Executing validated subprocess",
            extra={
                "command": cmd[0],
                "arg_count": len(cmd) - 1,
                "cwd": cwd,
        ...
        logger.error(
            "Subprocess timeout", extra={"command": cmd[0], "timeout": timeout}
        )
        raise ToolError(f"Command '{cmd[0]}' timed out after {timeout}s") from e

    except (OSError, subprocess.SubprocessError) as e:
        logger.error(
            "Subprocess execution failed", extra={"command": cmd[0], "error": str(e)}
        )
